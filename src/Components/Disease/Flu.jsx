...
// COUGH,MUSCLE_ACHES,TIREDNESS,SORE_THROAT,RUNNY_NOSE,STUFFY_NOSE,FEVER,NAUSEA,VOMITING,DIARRHEA,SHORTNESS_OF_BREATH,DIFFICULTY_BREATHING,LOSS_OF_TASTE,LOSS_OF_SMELL,ITCHY_NOSE,ITCHY_EYES,ITCHY_MOUTH,ITCHY_INNER_EAR,SNEEZING,PINK_EYE,TYPE
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000//api/flu", {
        flu: inputFormData,
      });
      console.log(response);
      setData(response.data["flu"]);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="w-full h-screen grid place-items-center">
      ...
      {!loading ? (
        state === "input" ? (
          <div className="w-1/2 mx-auto p-6 bg-gray-600 rounded-md shadow-md animate-slide-right">
